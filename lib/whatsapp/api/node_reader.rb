    ...
    class NodeReader
      ...
      def next_tree_internal
        ...
        if token == 1
          ...
          return Whatsapp::Api::Node.new('start', attributes)
        elsif token == 2
          return nil
        end
        ...
        return Whatsapp::Api::Node.new(tag, attributes, read_list(token)) if is_list_tag(token)

        Node.new(tag, attributes, [], read_string(token))
      end
