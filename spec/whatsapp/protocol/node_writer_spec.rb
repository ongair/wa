...
describe WhatsApp::Protocol::NodeWriter do
  ...
  it 'should write empty node' do
    @writer.write(nil).must_equal "\x10\x00\x05\xa2\xa8\x65\x3d\x62".b
  end

  it 'should start stream' do
    stream = @writer.start_stream('my-domain', 'my-resource')

    stream.must_equal "\x57\x41\x01\x02\x10\x00\x21\x5a\xdc\xf1\xcf\xef\x42\xaf\xc3\xda\x80\x89\xc7\x25\x1a\x62\xee\xec\x48\x18\xfa\x0b\x73\xe2\x80\x55\xa7\xee\x51\xa7\xdb\x93\xdd\xbd".b
  end

  it 'should flush buffer' do
    stream = @writer.send :flush_buffer

    stream.must_equal "\x10\x00\x04\x3a\x40\x00\x85".b
  end

  it 'should write 8-bit integer' do
    @writer.send(:write_int8, 154).must_equal "\x9a".b
  end

  it 'should write 16-bit integer' do
    @writer.send(:write_int16, 55534).must_equal "\xd8\xee".b
  end

  it 'should write 24-bit integer' do
    @writer.send(:write_int24, 15777214).must_equal "\xf0\xbd\xbe".b
  end

  it 'should write bytes' do
    @writer.send :write_bytes, "Some bytes \x01\x02\x03"

    @writer.output.must_equal "\xfc\x0e\x53\x6f\x6d\x65\x20\x62\x79\x74\x65\x73\x20\x01\x02\x03".b
  end
  ...
  it 'should write bytes containing unicode characters' do
    ...
    @writer.output.must_equal "\xfc\x31\x53\x6f\x6d\x65\x20\x62\x79\x74\x65\x73\x20\x01\x02\x03\x20\x61\x6e\x64\x20\x75\x6e\x69\x63\x6f\x64\x65\x20\x63\x68\x61\x72\x61\x63\x74\x65\x72\x73\x20\x6c\x69\x6b\x65\x20\xc5\x83\xc5\x9a\xc5\x81".b
  end
  ...
  it 'should write token' do
    ...
    @writer.output.must_equal "\xf4\xfe\x00".b
  end

  it 'should write jid' do
    @writer.send :write_jid, 'my-username', 'my-domain'

    @writer.output.must_equal "\xfa\xfc\x0b\x6d\x79\x2d\x75\x73\x65\x72\x6e\x61\x6d\x65\xfc\x09\x6d\x79\x2d\x64\x6f\x6d\x61\x69\x6e".b
  end

  it 'should write jid without username' do
    @writer.send :write_jid, nil, 'my-domain'

    @writer.output.must_equal "\xfa\x00\xfc\x09\x6d\x79\x2d\x64\x6f\x6d\x61\x69\x6e".b
  end
  ...
  it 'should write string of known tokens' do
    ...
    @writer.output.must_equal "\x5c\xfe\x03".b
  end

  it 'should write string of unknown tokens' do
    @writer.send :write_string, 'some-random-string'

    @writer.output.must_equal "\xfc\x12\x73\x6f\x6d\x65\x2d\x72\x61\x6e\x64\x6f\x6d\x2d\x73\x74\x72\x69\x6e\x67".b
  end
  ...
  it 'should write hash' do
    ...
    @writer.output.must_equal "".b
  end
  ...
  it 'should write list start' do
    ...
    @writer.output.must_equal "\x00\xf8\xff\xf9\xd8\xee".b
  end
